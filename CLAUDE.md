...
- Encre menstruelle du ruban : #db2f24 (`INK_COLORS` dans lib/ink.ts) au lieu du token #e2543f — le filament éclaircit ×1,28 avec écrêtage, ce qui virait le rouge au corail ; le pigment part donc d'un rouge saturé (vert/bleu bas). Étape intermédiaire #b23122 abandonnée : trop sombre, sa nappe floue restait terne face aux halos lumineux des autres phases (menstruel « en retrait »). #db2f24 rayonne autant qu'elles sans virer corail. L'accent UI reste --c-menst. Prototype aligné.
